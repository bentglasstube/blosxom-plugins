...
$plugin_dir = "/var/www/html/molelog2/plugins";
...
if ( $plugin_dir and opendir PLUGINS, $plugin_dir ) {
  foreach my $plugin ( grep { /^\w+$/ && -f "$plugin_dir/$_"  } sort readdir(PLUGINS) ) {
    ...
    t_start("plugin_whole_$plugin_name");
    t_start("plugin_load_$plugin_name");
    require "$plugin_dir/$plugin";
    t_end("plugin_load_$plugin_name");
    t_start("plugin_start_$plugin_name");
    $plugin_name->start() and ( $plugins{$plugin_name} = $on_off ) and push @plugins, $plugin_name;
    t_end("plugin_start_$plugin_name");
    t_end("plugin_whole_$plugin_name");
  }
  closedir PLUGINS;
}
t_end("plugin_start");
...
my %files = %$files; my %indexes = %$indexes;
t_end("entries");
t_start("plugin_filter");
# Plugins: Filter
foreach my $plugin ( @plugins ) { $plugins{$plugin} > 0 and $plugin->can('filter') and scalar(t_start("plugin_whole_$plugin"), t_start("plugin_filter_$plugin"), $entries = $plugin->filter(\%files), t_end("plugin_filter_$plugin"), t_end("plugin_whole_$plugin"))}
t_end("plugin_filter");
t_start("param");
# Static
...
# Dynamic
  ...
  t_end("content_type");
  t_start("print_generate");
  print generate('dynamic', $path_info, "$path_info_yr/$path_info_mo_num/$path_info_da", $flavour, $content_type);
  t_end("print_generate");
}
t_start("plugin_end");
# Plugins: End
foreach my $plugin ( @plugins ) { $plugins{$plugin} > 0 and $plugin->can('end') and scalar(t_start("plugin_whole_$plugin"), t_start("plugin_end_$plugin"), $entries = $plugin->end(), t_end("plugin_end_$plugin"), t_end("plugin_whole_$plugin"))}
t_end("plugin_end");
t_end('whole');
t_report();
...
sub generate {
  ...
  t_end("get_interpolate");
  t_start("head");
  # Head
  my $head = (&$template($currentdir,'head',$flavour));
  t_start("plugin_head");
  # Plugins: Head
  foreach my $plugin ( @plugins ) { $plugins{$plugin} > 0 and $plugin->can('head') and scalar(t_start("plugin_whole_$plugin"), t_start("plugin_head_$plugin"), $entries = $plugin->head($currentdir, \$head), t_end("plugin_head_$plugin"), t_end("plugin_whole_$plugin"))}
  t_end("plugin_head");

  t_start("interpolate_head");
  $head = &$interpolate($head);
  t_end("interpolate_head");
  ...
  t_start("foreach_story");
  foreach my $path_file ( @sorted) {
    last if $ne <= 0 && $date !~ /\d/;
    use vars qw/ $path $fn /;
    ...
    # Plugins: Date
    t_start("plugin_date");
    foreach my $plugin ( @plugins ) { $plugins{$plugin} > 0 and $plugin->can('date') and scalar(t_start("plugin_whole_$plugin"), t_start("plugin_date_$plugin"), $entries = $plugin->date($currentdir, \$date, $files{$path_file}, $dw,$mo,$mo_num,$da,$ti,$yr), t_end("plugin_date_$plugin"), t_end("plugin_whole_$plugin")) }
    t_end("plugin_date");

    t_start("interpolate_date");
    $date = &$interpolate($date);
    t_end("interpolate_date");
    ...
    t_start("plugin_story");
    # Plugins: Story
    foreach my $plugin ( @plugins ) { $plugins{$plugin} > 0 and $plugin->can('story') and scalar (t_start("plugin_whole_$plugin"), t_start("plugin_story_$plugin"), $entries = $plugin->story($path, $fn, \$story, \$title, \$body), t_end("plugin_story_$plugin"), t_end("plugin_whole_$plugin")) }
    t_end("plugin_story");

    t_start("story_escape");
    if ($content_type =~ m{\Wxml$ }x) {
    ...
    t_end("story_escape");
    t_start("interpolate_story");
    $story = &$interpolate($story);
    t_end("interpolate_story");
    $output .= $story;
    $fh->close;
    t_end("story");
    $ne--;
  ...
  t_start("plugin_foot");
  foreach my $plugin ( @plugins ) { $plugins{$plugin} > 0 and $plugin->can('foot') and scalar(t_start("plugin_whole_$plugin"), t_start("plugin_foot_$plugin"), $entries = $plugin->foot($currentdir, \$foot), t_end("plugin_foot_$plugin"), t_end("plugin_whole_$plugin")) }
  t_end("plugin_foot");

  t_start("interpolate_foot");
  $foot = &$interpolate($foot);
  $output .= $foot;
  t_end("interpolate_foot");
...
my %t;
sub t_start {
  warn "restarting existing timer $_[0]" if exists($t_start{$_[0]});
  $t_start{shift @_} = [Time::HiRes::gettimeofday];
  1;
}
sub t_end {
  $t{$_[0]} += Time::HiRes::tv_interval($t_start{$_[0]});
  delete $t_start{$_[0]};
  1;
}
sub t_report {
	...
	"\n";
    warn "existing timer $_" foreach keys %t_start; 
}
...
html foot <p /><center><a href="http://www.raelity.org/apps/blosxom/"><img src="http://www.raelity.org/images/pb_blosxom.gif" border="0" /></body></html>
